...
/**
 * Split a block of SQL text into individual statements on ';', while
 * ignoring semicolons that appear inside '...', "...", or `...` so that
 * the SQL Runner can accept multi-statement scripts the same way
 * phpMyAdmin's SQL tab does.
 */
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["sql_executor_action"])) {
    ...
    // Split on ';' so a pasted multi-statement script (like phpMyAdmin accepts)
    // runs as a sequence of individual statements instead of failing outright.
    ...
    foreach ($statements as $singleSql) {
        try {
            $stmt = $pdo->prepare($singleSql);
            $stmt->execute();

            $firstKeyword = strtoupper(strtok(ltrim($singleSql), " \t\n"));
            ...
            // Stop on the first failing statement, same as phpMyAdmin's default behavior,
            // so a typo early in a script doesn't silently skip ahead to later statements.
            $stoppedEarly = true;
            break;
        }
    }
...
<head>
    <meta charset="UTF-8">
    <title>Developer Database Manager — TeamInternGrub</title>
...
<body>
    ...
    <div class="page-wrapper">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <div class="text-end">
                <a href="<?= BASE_URL ?>/dashboard.php" class="btn btn-sm btn-outline-secondary me-2">← Back to TeamInternGrub</a>
                <span class="badge bg-danger">Development Tool</span>
                <p class="text-muted small mb-0 mt-1">Admin-only. Remove before going live, or right after you're done testing.</p>
            </div>
        </div>
        ...
        <div class="row g-4">
            ...
            <div class="col-lg-9">
                ...
                <?php elseif ($actionMode === "sql" && $selectedTable === ""): ?>
                    ...
                    <div class="table-card">
                        ...
                        <form method="POST" action="database.php">
                            ...
                            <div class="mb-3">
                                ...
                                <small class="text-muted">Supports SELECT, INSERT, UPDATE, DELETE, CREATE, DROP, ALTER, SHOW, DESCRIBE, and more. You can paste multiple statements separated by <code>;</code>, just like phpMyAdmin's SQL tab — they'll run in order, and execution stops at the first one that errors.</small>
                            </div>
                            <div class="d-flex gap-2">
                    ...
                    <?php if ($sqlResults && ($sqlResults["type"] ?? "") === "batch"): ?>
                        <div class="table-card">
                            <div class="d-flex justify-content-between align-items-center mb-2">
                                <h6 class="fw-bold mb-0">Result</h6>
                                <span class="text-muted small">
                                    Ran <?= (int)$sqlResults["ranCount"]; ?> of <?= (int)$sqlResults["totalCount"]; ?> statement(s)
                                    <?php if ($sqlResults["stoppedEarly"]): ?>
                                        · <span class="text-danger fw-semibold">stopped at the first error</span>
                                    <?php endif; ?>
                                </span>
                            </div>

                            <?php foreach ($sqlResults["results"] as $i => $singleResult): ?>
                                <div class="border rounded p-3 mb-2" style="background:#fafbfc;">
                                    <div class="d-flex justify-content-between align-items-start mb-2">
                                        <span class="badge bg-secondary">#<?= $i + 1; ?></span>
                                    </div>
                                    <pre class="bg-light rounded p-3 mb-2" style="font-size:13px;white-space:pre-wrap;"><?= h($singleResult["sql"]); ?></pre>

                                    <?php if ($singleResult["type"] === "error"): ?>
                                        <div class="alert alert-danger mb-0"><strong>Error:</strong> <?= h($singleResult["error"]); ?></div>
                                    <?php elseif ($singleResult["type"] === "exec"): ?>
                                        <div class="alert alert-success mb-0">Query executed successfully. <strong><?= $singleResult["affected"]; ?> row(s) affected.</strong></div>
                                    <?php elseif ($singleResult["type"] === "select"): ?>
                                        <?php if (empty($singleResult["rows"])): ?>
                                            <div class="alert alert-info mb-0">Query returned no rows.</div>
                                        <?php else: ?>
                                            <p class="text-muted small mb-2"><?= number_format($singleResult["count"]); ?> row(s) returned.</p>
                                            <div class="table-responsive">
                                                <table class="table table-bordered table-sm table-hover small-table-preview mb-0">
                                                    <thead class="table-dark">
                                                        <tr>
                                                            <?php foreach (array_keys($singleResult["rows"][0]) as $col): ?>
                                                                <th><?= h($col); ?></th>
                                                            <?php endforeach; ?>
                                                        </tr>
                                                    </thead>
                                                    <tbody>
                                                        <?php foreach ($singleResult["rows"] as $row): ?>
                                                            <tr>
